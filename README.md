...
主同步（16:05）時順便拍當日部位，寫入**分年度、自動建立**的工作表。
中信無此 API，不納入。

- **每日持股 YYYY**（單一 tab）：日期, 帳戶, 股名, 股數, 平均成本, 市價, 市值, 未實現損益, 未實現損益率
- **每日總覽-<帳戶> YYYY**（每帳戶一張，方便各自畫資產曲線）：日期, 持股市值, 現金, 未交割淨額, 帳戶總值, 未實現損益, 入金出金, 累計淨注資, 真實累計損益

> **帳戶總值 = 持股市值 + 現金 + 未交割淨額**（含 T+2 交割款；現金為當下餘額尚未扣未交割）。
> `入金出金` 手動填（入 +、出 −）；`真實累計損益 = 帳戶總值 − 起始帳戶總值 − 累計淨注資`
> （baseline = 該 tab 第一列，跨年自動用當年第一列）。股利/折讓是真收益、自動計入，不用標記。
> 只在當日執行時快照、同日不覆蓋、沒成交也記一筆。
